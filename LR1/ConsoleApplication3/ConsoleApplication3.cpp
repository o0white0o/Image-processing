...
int main(int argc, char** argv)
{
	...
	Mat copyHist;																// Копия гистограммы
	...
	double FivePercent = round((double)(image.cols*image.rows)*0.10);			// 5% от общего числа пикселей исходного изображения
	...
	{
		image.at<unsigned char>(i) = (image.at<unsigned char>(i) - a)*((d - c) / (b - a)) + c;
	}
	imshow("After", image);
	...
	vector<float> LT;
	LT.push_back(double(image2.at<unsigned char>(0)) / (image2.rows*image2.cols));
	for (int i = 1; i < hist.rows*hist.cols; i++)
	{
		LT.push_back(LT.at(i - 1) + double(image2.at<unsigned char>(i)) / (image2.rows*image2.cols));
	}
	for (int i = 0; i < image2.rows*image2.cols; i++)
	{
		image2.at<unsigned char>(i) = LT.at(image2.at<unsigned char>(i));
	}
